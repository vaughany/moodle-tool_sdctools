...
echo $OUTPUT->heading( get_string( 'leapcorereport', 'tool_sdctools' ) . $course->fullname );
...
foreach ( $lc_types as $lc_type ) {
    ...
    if ( $lc_res ) {
        ...
        $out .= '<p>No courses have ' . $lc_type . ' in the "ID number" field.</p>'."\n";
    }
...
$lc_total = $leapcore_core + $leapcore_english + $leapcore_maths + $leapcore_ppd;

echo '<p>Total courses with LEAPCORE set: ' . $lc_total . ".</p>\n";
